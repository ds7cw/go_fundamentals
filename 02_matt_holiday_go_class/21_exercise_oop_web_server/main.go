...
func runServer() {
	...
	// $ curl localhost:8080/create?item=shirt\&price=20
	http.HandleFunc("/update", db.update)
	// $ curl localhost:8080/update?item=shirt\&price=25
	http.HandleFunc("/delete", db.drop)
	// $ curl localhost:8080/delete?item=shoes -i
	http.HandleFunc("/read", db.fetch)
	// $ curl localhost:8080/read?item=shoes -i
